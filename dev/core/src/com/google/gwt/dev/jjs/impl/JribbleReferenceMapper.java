 ...
 *
 * An instance of this class can be reused to create several mini ASTs, as we can recycle
 * the external JFields, JMethods, and JReferenceTypes. The non-external
 * ASTs are flushed after each CompilationUnit is built.
 *
 * Closely modeled after the original {@ReferenceMapper}.
 */
public class JribbleReferenceMapper {
  ...
  public JType getType(String name) {
    ...
    throw new RuntimeException("No existing type " + name);
  }
  
  /** assumes the caller knows name is a class type */
  public JClassType getClassType(String name) {
    ...
    // ReferenceMapper fills in super class, interfaces, and clinit
  ...
  public JInterfaceType getInterfaceType(String name) {
    ...
    JInterfaceType intType = new JInterfaceType(name);
    types.put(name, intType);
  ...
  private JType getExistingType(String name) {
    ...
    if (externalType != null) {
      assert externalType instanceof JPrimitiveType || externalType == JNullType.INSTANCE || externalType.isExternal();
      return externalType;
    }
    return null;
  }
